...
if (!empty($_GET['shop']) && !empty($_GET['code'])) {
    ...
    if (checkShopSettings($shop) == false) {
        $db->query("insert into product_faqs_settings(shop) values('$shop')");
    }
    createShopSettingsFile($shop, $shopify);
    ...
    if ($putjs1) {
        foreach ($putjs1 as $value) {
            if ($value["src"] == $rootLink . '/productfaqs.js') {
                $check = false;
                break;
            }
        }
    ...
    if ($check) {
        $putjs = $shopify('POST', '/admin/script_tags.json', array('script_tag' => array('event' => 'onload', 'src' => $rootLink . '/productfaqs.js')));
    }
